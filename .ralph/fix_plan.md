...
### Completed (Epic 26 — Documentation Axis: Stories 26.1 + 26.2)

- [x] Stories 26.1 + 26.2: Fondations + Core Integration (2026-03-18)
  - `src/core/axes/documentation.ts`: DocumentationEvaluator (Haiku), DocumentationResponseSchema, DocsCoverageSchema, buildDocumentationUserMessage()
  - `src/core/axes/prompts/documentation.system.md`: JSDoc evaluation rules + concept coverage + output format
  - `src/core/docs-resolver.ts`: buildDocsTree() + resolveRelevantDocs() (config mapping + convention fallback, max 3 pages × 300 lines)
  - `src/core/axis-evaluator.ts`: AxisId +'documentation', AxisContext +docsTree/relevantDocs, RelevantDoc interface
  - `src/schemas/review.ts`: SymbolReviewSchema +documentation enum, AxisIdSchema +'documentation'
  - `src/schemas/config.ts`: DocumentationConfigSchema (docs_path, module_mapping), AxesConfigSchema +documentation
  - `src/core/axis-merger.ts`: AXIS_DEFAULTS +'documentation', mergeSymbol() documentation resolution, coherence DEAD→UNDOCUMENTED, action synthesis (UNDOCUMENTED medium, PARTIAL low), computeVerdict (UNDOCUMENTED exported + 3+ PARTIAL)
  - `src/core/file-evaluator.ts`: docsTree in EvaluateFileOptions, resolveRelevantDocs per file
  - `src/commands/run.ts`: buildDocsTree() in setup, docsTree threaded to reviewPhase
  - `src/core/reporter.ts`: Doc column in shard findings table, Documentation in axis summary + methodology + coherence rules
  - `src/core/axes/index.ts`: DocumentationEvaluator registered, ALL_AXIS_IDS includes 'documentation'
...
- **Epic 26 dependency order:** 26.1 → 26.2 → 26.3 → 26.4 (26.3 can start in parallel with 26.2 for doc-only tasks)
