...
The axis evaluator system is the core review engine of Anatoly. Each file that passes triage is evaluated through 6 independent axis evaluators running in parallel. Each axis focuses on a single quality dimension, receives its own LLM call with a specialised prompt, and returns structured JSON results validated against a Zod schema.
...
file-evaluator.ts          Orchestrates all axes for one file
  ...
  |
  +-- axis-merger.ts        Combines 6 axis results into one ReviewFile
```
...
- If one axis crashes (SDK error, timeout, validation failure), the other 5 axes still complete successfully.
...
2. **Errors override complexity**: If `correction = ERROR`, force `overengineering = ACCEPTABLE`.
...
- **NEEDS_REFACTOR**: At least one symbol has a confirmed finding (NEEDS_FIX, DEAD, DUPLICATE, or OVER) with confidence >= 60.
...
- Individual axes: `src/core/axes/utility.ts`, `duplication.ts`, `correction.ts`, `overengineering.ts`, `tests.ts`, `best-practices.ts`
