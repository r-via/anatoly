...
import { autoDetectScanGlobs } from '../core/scan-autodetect.js';
 ...
 */
export async function writeFirstRunConfig(
  projectRoot: string,
  ...
  // Detect the project profile once and derive concrete scan globs from it.
  // Failures (no git, unreadable manifests) fall back to a permissive default
  // inside autoDetectScanGlobs — the wizard never blocks on detection.
  let scanGlobs;
  try {
    const profile = await detectProjectProfile(projectRoot);
    scanGlobs = autoDetectScanGlobs(projectRoot, profile);
  } catch {
    scanGlobs = { include: ['src/**/*'], exclude: ['node_modules/**', 'dist/**'] };
  }

  const yamlBody = buildAnnotatedV3Yaml(tier, hasApiKey, scanGlobs);
...
function buildAnnotatedV3Yaml(
  tier: 'lite' | 'advanced' | 'external',
  hasApiKey: boolean,
  scanGlobs: { include: string[]; exclude: string[] },
): string {
  return [
    ...
    activeYaml({
      scan: {
        include: scanGlobs.include,
        exclude: scanGlobs.exclude,
...
function renderProvidersBlock(
  ...
  // `local-lite` is always active regardless of tier — it's the universal
  // fallback (ONNX in-process, no API key) so embeddings can degrade gracefully
  // even when the user picked advanced or external. Its models are only
  // *used* when routing.embeddings refers to it.
  const active: Record<string, unknown> = {
    anthropic: anthropicProviderConfig(hasApiKey),
    'local-lite': LOCAL_LITE_PROVIDER,
  };
