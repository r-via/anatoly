 ...
 *
 * Emits a v3 schema (declarative `version: 3` format with explicit providers,
 * routing slots, evaluation axes, and runtime knobs). The tier dictates which
 * embedding provider gets declared and pinned in `routing.embeddings`:
 *
 * - `lite` → `local-lite` provider (ONNX in-process, Jina + MiniLM)
 * - `advanced` → `local-advanced` provider (Docker GGUF sidecar on localhost)
 * - `external` → `mistral` + `openrouter` providers (third-party APIs).
...
export function writeFirstRunConfig(
  ...
  const config = buildV3Config(tier, hasApiKey);
  const yamlBody = yaml.dump(config, { lineWidth: 120, noRefs: true });
  const header = '# Anatoly configuration — generated by first-run wizard. Edit freely.\n\n';
  writeFileSync(resolve(projectRoot, '.anatoly.yml'), header + yamlBody, 'utf-8');

  if (tier === 'external') writeExternalReadyFlag(projectRoot);
}

/**
 * Build the v3 config object for a given tier. Returned as a plain JS object
 * so `yaml.dump` can serialize it without re-implementing each section by
 * hand. The shape mirrors `ConfigV3Schema` exactly.
 */
function buildV3Config(
  tier: 'lite' | 'advanced' | 'external',
  hasApiKey: boolean,
): Record<string, unknown> {
  const anthropicProvider: Record<string, unknown> = hasApiKey
    ? { transport: 'claude_agent_sdk', auth: 'api_key', env_key: 'ANTHROPIC_API_KEY', concurrency: 24,
        models: [DEFAULT_MODELS_V3.quality, DEFAULT_MODELS_V3.fast, DEFAULT_MODELS_V3.deliberation] }
    : { transport: 'claude_agent_sdk', auth: 'oauth', concurrency: 24,
        models: [DEFAULT_MODELS_V3.quality, DEFAULT_MODELS_V3.fast, DEFAULT_MODELS_V3.deliberation] };

  const providers: Record<string, unknown> = { anthropic: anthropicProvider };
  let embeddingsCode: string;
  let embeddingsText: string;

  if (tier === 'lite') {
    providers['local-lite'] = {
      transport: 'onnxruntime_node',
      models: ['jinaai/jina-embeddings-v2-base-code', 'Xenova/all-MiniLM-L6-v2'],
    };
    embeddingsCode = 'local-lite/jinaai/jina-embeddings-v2-base-code';
    embeddingsText = 'local-lite/Xenova/all-MiniLM-L6-v2';
  } else if (tier === 'advanced') {
    providers['local-advanced'] = {
      transport: 'openai_compatible',
      auth: 'api_key',
      env_key: 'ANATOLY_LOCAL_DUMMY_KEY',  // sidecar accepts any value
      base_url: 'http://localhost:8082/v1',
      models: ['nomic-embed-code-gguf', 'qwen3-embedding-8b-gguf'],
    };
    embeddingsCode = 'local-advanced/nomic-embed-code-gguf';
    embeddingsText = 'local-advanced/qwen3-embedding-8b-gguf';
  } else {
    providers.mistral = {
      transport: 'openai_compatible',
      auth: 'api_key',
    ...
    embeddingsCode = 'mistral/mistral-embed';
    embeddingsText = 'openrouter/qwen/qwen3-embedding-8b';
  }
  ...
  return {
    ...
    evaluation: {
      axes: Object.fromEntries(AXIS_IDS.map((id) => [id, true])),
    },
  };
}
